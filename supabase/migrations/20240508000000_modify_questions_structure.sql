-- Añadir columna para rastrear uso de preguntas (solo si no existe)
ALTER TABLE questions 
ADD COLUMN IF NOT EXISTS times_used INTEGER DEFAULT 0;

-- Crear tabla game_questions solo si no existe
...
-- Función para inicializar preguntas de juego
CREATE OR REPLACE FUNCTION initialize_game_questions(p_game_id UUID, total_rounds INTEGER DEFAULT 7)
RETURNS VOID AS $$
DECLARE
  question_record RECORD;
  current_round INTEGER;
  category_array TEXT[] := ARRAY['pelicula', 'sigla', 'personaje', 'pelicula', 'sigla', 'personaje', 'pelicula'];
